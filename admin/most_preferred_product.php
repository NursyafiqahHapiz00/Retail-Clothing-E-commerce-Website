	...
	<div class="row"> <!-- Row 1 starts -->
		<div class="col-lg-12"> <!-- To make website responsive -->
			<ol class="breadcrumb"> <!-- To create horizontal navigation bar -->
				<li class="active">
					<i class="fa fa-dashboard"></i> Dashboard / Reports / Most Preferred Products Report
				</li>
			</ol>
		</div>
	</div>
			...
			<div class="panel panel-default"> <!-- To colour the style of the panel  -->
				<div class="panel-heading"> <!-- To create a padding box around the heading of the coupon panel -->
					<h3 class="panel-title">
						<i class="fa fa-print"></i> Most Preferred Products Report
					</h3>
				</div>	
								...
								<div class="input-group col-md-4">
									<input type="text" class="form-control" placeholder="Search here..." name="search"  value="<?php if(isset($_POST['search'])){echo $_POST['search']; } ?>"/>
										<span class="input-group-btn">
											...
											<div class="col-md-2">
												<a href="index.php?most_preferred_product" type="button" class="btn btn-success">Refresh</a>
											</div>
								</div>
							</form>
                            <br><br>
								...
								<table class="table table-bordered table-hover table-striped" > <!-- To create table striped -->
									...
									<?php 	
										if(isset($_POST['search'])) {
											...
											$i=0;
											$query = "select * from pending_orders t1 inner join products t2 on t1.product_id=t2.product_id inner join cust_feedbacks t3 on t1.invoice_no=t3.invoice_no where CONCAT(order_id,order_qty >= 5) like '%$filtervalues%' ";
                                    ...
                                    ?>
                                        ...
                                        <tr>
                                            <?php
                                                $query=mysqli_query($con, "select count(product_id) as sum from `pending_orders` where CONCAT(order_id,product_id,order_qty >= 5) like '%$filtervalues%'");
                                                    while($fetch=mysqli_fetch_array($query)){
                                            ?>

                                            <td colspan = "4"> <b><?php echo "Number of Most Preferred Products";?></b> </td>
                                    ...
                                    <?php
										...
										echo '
											...
											//Refresh page
												...
												$i=0;
												$query = "select * from pending_orders t1 inner join products t2 on t1.product_id=t2.product_id inner join cust_feedbacks t3 on t1.invoice_no=t3.invoice_no where order_qty >= 5";
												$query_run = mysqli_query($con, $query);
                                    ...
                                    ?>
                                        ...
                                        <tr>
                                            <?php
                                                $query=mysqli_query($con, "select count(product_id) as sum from `pending_orders` where order_qty >= 5");
                                                    while($fetch=mysqli_fetch_array($query)){
                                            ?>
