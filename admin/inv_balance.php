	...
	<div class="row"> <!-- Row 1 starts -->
		<div class="col-lg-12"> <!-- To make website responsive -->
			<ol class="breadcrumb"> <!-- To create horizontal navigation bar -->
				<li class="active">
					<i class="fa fa-dashboard"></i> Dashboard / Reports / Low Inventory Report
				</li>
			</ol>
		</div>
	</div>
			...
			<div class="panel panel-default"> <!-- To colour the style of the panel  -->
				<div class="panel-heading"> <!-- To create a padding box around the heading of the coupon panel -->
					<h3 class="panel-title">
						<i class="fa fa-print"></i> Low Inventory Report
					</h3>
				</div>	
								...
								<div class="input-group col-md-4">
									<input type="text" class="form-control" placeholder="Search here..." name="search"  value="<?php if(isset($_POST['search'])){echo $_POST['search']; } ?>"/>
										<span class="input-group-btn">
											...
											<div class="col-md-2">
												<a href="index.php?inv_balance" type="button" class="btn btn-success">Refresh</a>
											</div>
								</div>
							</form>
                            <br><br>
                            ...
                            <div class="input-group col-md-6">
                                <div class="box-title"><b>Status :</b> To Be Restocked for Quantity Less Than 50</div>
						    </div>
                            <br>
								...
								<table class="table table-bordered table-hover table-striped" > <!-- To create table striped -->
									...
									<?php 	
										if(isset($_POST['search'])) {
											...
											$i=0;
											$query = "select * from product_variations t1 inner join products t2 on t1.product_id=t2.product_id where CONCAT(p_variations_id,product_variations,product_size,product_quantity <= 50) like '%$filtervalues%' ";
                                    ...
                                    ?>
                                        ...
                                        <tr>
                                            <?php
                                                $query=mysqli_query($con, "select sum(product_quantity) as sum from `product_variations` where CONCAT(p_variations_id,product_variations,product_size,product_quantity <= 50) like '%$filtervalues%'");
                                                    while($fetch=mysqli_fetch_array($query)){
                                            ?>

                                            <td colspan = "5"> <b><?php echo "Number of Products to Be Restocked";?></b> </td>
                                    ...
                                    <?php
										...
										echo '
											...
											//Refresh page
												...
												$i=0;
												$query = "select * from products t1 inner join product_variations t2 on t1.product_id=t2.product_id where product_quantity <= 50";
												$query_run = mysqli_query($con, $query);
                                    ...
                                    ?>
                                        ...
                                        <tr>
                                            <?php
                                                $query=mysqli_query($con, "select sum(product_quantity) as sum from `product_variations` where product_quantity <= 50");
                                                    while($fetch=mysqli_fetch_array($query)){
                                            ?>
